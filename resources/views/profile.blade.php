		...
		<div class="row" style="margin-bottom: 0px">
		    ...
		    <div id="test2" class="col s12">
				...
				<div class="card feed z-depth-4">
		            ...
		            <div class="card-content" style="padding-top: 10px;padding-bottom: 5px">
		              <div class="row" style="padding-top: 5px; margin-bottom: 10px">
			              ...
			              @if ($likedpost->user_id==$user->id)
			                <button class="btn-floating right delButton" id="delButton" value="{{$likedpost->id}}" ><i id="{{$likedpost->id}}delspinner" class="fa fa-trash" style="font-size: 16px"></i></button>
			               @endif
			              </div>
		            </div>
		              <blockquote style="text-align: justify;margin-top: 0px;margin-bottom: 10px">
		              ...
		              <a id="{{$likedpost->id}}likes" data-id="{{$likedpost->id}}" class="chip likescount blue white-text" style="padding: 6px;display: inline;cursor: pointer;">{{$likedpost->likes()->count()}} Likes</a>
		              <a id="{{$likedpost->id}}comments" data-id="{{$likedpost->id}}" class="chip commentscount blue white-text" style="padding: 6px;cursor: pointer;display:inline">{{$likedpost->comments()->count()}} Comments</a>
		            ...
		            <div class="card-action" style="padding-top: 0px;padding-bottom: 0px; ">
			            <div class="row" style="padding-top: 0px;margin-bottom: auto;">
			              <div class="col s2 m1" style="padding-top: 10px">
			                @if($likes->contains('post_id',$likedpost->id))
							<a style="cursor: pointer;margin-right: 0px;" class="likebutton left" value="{{$likedpost->id}}"><i class="material-icons" style="font-size:32px">favorite</i></a>	
							@else
							<a style="cursor: pointer;margin-right: 0px;" class="likebutton left" value="{{$likedpost->id}}"><i class="material-icons" style="font-size:32px">favorite_border</i></a>
							@endif
			              </div>
			              <div class="col s8 m10">
			...
			<div class="card-content">
			    ...
			    ?>
					<blockquote>
			    		@if ($activity->type==0)
			    			You <a style="cursor: pointer;" data-id="1" data-pid="{{$activity->post_id}}" class="viewStoryBtn">posted</a> on the wall.
			    		@elseif ($activity->type==1)
		    				You liked {{$postedon}} <a style="cursor: pointer;" data-id="1" data-pid="{{$activity->post_id}}" class="viewStoryBtn">post.</a>
			    		@else
		    				You commented on {{$postedon}} <a style="cursor: pointer;" data-id="1" data-pid="{{$activity->post_id}}" class="viewStoryBtn">post.</a>
			    		@endif
			    	</blockquote>
				@endforeach
			</div>
